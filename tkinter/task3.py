...
def draw_rect():
    ...
    try:
        ...
        if a_float < 0 or b_float < 0:
            raise ValueError('Both numbers must be greater than 0 when drawing rectangle!')
        
        rect_canvas.create_rectangle(0, 0, a_float, b_float, fill=random_color(), outline="", stipple='gray75')
    except:
        pass
...
if __name__ == '__main__':
    ...
    rect_canvas = Canvas(content, background='#1e1e1e')
    ...
    rect_canvas.grid(column=4, row=0, pady=5, padx=5, sticky=(N, E, S), columnspan=3)
